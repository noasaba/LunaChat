...
| Final accepted boundary | Final event-transformed string before render | `BukkitChannel` hook and `PaperIntegrationService` | wire round-trip/gateway tests | Implemented; process assertion needs a running Paper client/harness |
...
| Retry/dedup/replay | logical/frame split, fresh encryption, ACK, bounded windows | `ReliableOutbox`, replay and receipt maps | retry identity, replay, capacity recovery tests | Implemented in-process; ACK-loss multi-process gate awaits Docker |
